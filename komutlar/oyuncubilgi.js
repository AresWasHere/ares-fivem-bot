...
exports.run = (client, message, args, color) => {
  ...
  let sqlkademe = `SELECT * FROM users WHERE identifier = 'steam:${args[0]}'`;
     ...
     return;
                            ...
                            .setDescription(`Oyuncunun Adı Soyadı: `+ `**${results[0].firstname}**` + ` **${results[0].lastname}**` +     `\n Oyuncunun Telefon Numarası: `+ `**${results[0].phone_number}** \n` + `Oyuncunun Mesleği: `+ `**${results[0].job}**\n` + ` Oyuncunun Üstündeki Para Miktarı: `+ `**${results[0].money}** \n` + `Oyuncunun Banka Hesabındaki Para Miktarı: `+ `**${results[0].bank}**`  )
